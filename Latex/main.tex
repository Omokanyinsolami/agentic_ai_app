...
Agentic AI is relevant to this problem because it refers to systems that can pursue goals with bounded autonomy, use tools, reason about changing context, and adapt behaviour based on feedback \parencite{10849561,hughes2025ai}. In the context of academic task management, this means an AI system can move beyond passive storage and act as a purposeful support layer: it can help interpret task data, suggest priorities, generate schedules, explain recommendations, and adapt plans when the learner's situation changes.

This dissertation therefore frames the project as the design and evaluation of a learner-support prototype rather than a narrow single-cohort planning tool. Although the original implementation context emerged from postgraduate study management, the target problem is broader: enabling learners in higher education and similar structured learning environments to manage tasks, deadlines, study time, and change more effectively.
...
\subsection{Dissertation Outline Mapping to the Handbook}
To ensure compliance with the MSc handbook structure, this dissertation is intentionally organised using a one-to-one mapping between handbook expectations and implemented sections of this report. The front matter contains the required title page and declaration statements, abstract, acknowledgements, table of contents, list of figures, list of tables, and list of acronyms. The main body then follows a technical narrative from problem framing to evidence-backed conclusions.

The first part of the body (Introduction and project description) defines the domain, motivation, research questions, hypotheses, scope boundaries, and success criteria. This aligns with the handbook expectation that the early chapter should function as a clear roadmap for the examiner, explaining both why the work matters and how the subsequent sections contribute to answering the project objectives.

The second part is a combined literature review and technical review. In line with supervisor feedback, the literature chapter is divided into digital and online educational support systems, AI systems (including agentic and non-agentic approaches), and a relevant software/framework overview. This dual role is deliberate because the project is both research-led and implementation-heavy. A purely conceptual review would be insufficient for a software artefact dissertation where architecture and tool-chain decisions materially affect validity, reproducibility, and measured outcomes.

The third part presents software and system design methods, including requirements elicitation, use cases, architecture decomposition, setup/configuration strategy, and evaluation methodology. This directly addresses the handbook requirement for methods and design alternatives. Throughout this part, decisions are justified in terms of technical feasibility, evidence quality, and trade-offs between deterministic behaviour and adaptive intelligence.

The fourth part documents development, setup/configuration, debugging, and testing evidence. Because this project contains backend APIs, frontend workflows, orchestration logic, and evaluation harnesses, testing is reported at multiple levels: unit validation, API contracts, integration behaviour, scenario-level simulation, and non-functional benchmarking. This corresponds to the handbook expectation that implementation claims must be supported by verifiable results rather than descriptive assertions.

The fifth part presents outcomes, results, and analysis. Quantitative results are combined with qualitative interpretation to show what improved, what remained constrained, and why. This is followed by a dedicated discussion and evaluation section that situates results in the wider literature, identifies limitations, and clarifies what can and cannot be inferred from the current prototype.

Finally, the report concludes with a focused final conclusions chapter and a separate future work chapter to conform to the handbook sequence. References and appendices close the document. This structure was selected so an examiner can evaluate the project linearly: motivation, prior work, method, build, evidence, interpretation, conclusions, and extensions.



\subsection{Project Scope and Objectives}
\noindent The project scope is a functional software prototype that actively supports learners in managing academic work. The prototype is designed for structured learning environments where users must handle deadlines, tasks, study availability, and changing priorities. It is not restricted to a single cohort; rather, postgraduate learners, undergraduate learners, and similar independent learners represent the intended support context.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item specify a structured system design for the software prototype, including architecture, data model, workflow logic, and purposeful agent behaviour.
...
The project follows the six-part work plan emphasised in supervisor feedback: (1) literature review, (2) requirements analysis and use cases, (3) system design, (4) setup/configuration and implementation, (5) testing, and (6) project evaluation. This sequencing is intentional because the running prototype now provides an implementation foundation, while the dissertation must demonstrate how that implementation solves a clearly defined learner-support problem and how its operational value is evidenced. A detailed project schedule with weekly breakdown is provided in Section~\ref{proj_sched_risk_mgt}.
...
What is often missing is a coherent supportive environment that actively combines these functions. Existing systems rarely integrate task capture, prioritisation, scheduling, conflict detection, explanation, and adaptation into one purposeful workflow. This creates a gap between digital access to academic information and genuine support for academic self-management.
...
\begin{enumerate}
    ...
    \item Agentic AI research has not yet provided enough operational evidence showing how purposeful learner-support behaviour can be implemented, tested, and evaluated in a reliable software prototype.
...
The handbook emphasises professional and legal quality, which in software projects requires explicit treatment of data integrity and access control. Therefore, a dedicated technical review was performed for persistence, transaction boundaries, and session security design.
...
This use-case-driven method was important for supervisor-aligned scope control. It ensured that the prototype was not described in abstract AI terms only, but in terms of what the system is actually expected to do for users. Each requirement therefore supports at least one concrete learner interaction that can later be tested and evaluated.
...
The governance method also informed documentation strategy: unresolved non-functional benchmarks are explicitly reported as pending rather than silently omitted. This conservative reporting reflects professional integrity expectations in postgraduate research work.
...
\begin{enumerate}
    ...
    \item \textbf{Reminder System:} SMTP-based email notifications for upcoming deadlines with friendly formatting
...
Supervisor feedback placed particular emphasis on setup/configuration because a prototype should be demonstrably operational rather than conceptually described. For that reason, implementation work in this project included not only feature coding but also configuration discipline, environment management, debugging, and reproducible execution pathways.
...
\begin{enumerate}
    ...
    \item environment-variable configuration for database access, API origins, session expiry, and optional external services.
\end{enumerate}
...
These solutions reinforce the dissertation's central claim: practical agentic systems benefit from disciplined software engineering boundaries. The final implementation therefore reflects not just feature completion, but an evidence-oriented development process that aligns with the handbook emphasis on design rationale, implementation quality, and evaluable outcomes.
...
\hline
FR15 & Implemented & Reminder generation and optional SMTP email sending. \\
\hline
...
\hline
UC6 & Request reminders or approaching-deadline support & Reminder content generated and optional email path remains available & Passed at prototype scope \\
\hline
\end{tabular}
...
\begin{itemize}
    ...
    \item proactive reminder generation with optional email delivery;
...
\hline
UC6 & Reminder generation and optional email delivery provide proactive support for approaching deadlines. & Fulfilled at prototype scope \\
\hline
\end{tabular}
...
Compared to classical AI planning systems \parencite{geffner2013concise,ghallab2016automated}, this implementation trades formal optimality guarantees for flexibility and ease of extension. Classical planners would provide stronger guarantees about schedule feasibility but would be more difficult to extend with natural language capabilities.

Compared to pure LLM-based agents \parencite{sun2023adaplanner,yao2022react}, this implementation achieves greater efficiency and determinism through graph-based routing. The trade-off is reduced flexibility in handling novel, unstructured requests.

Compared to existing intelligent scheduling systems \parencite{chen2023survey}, this implementation provides greater transparency through explicit state management and graph-based routing. The system's behaviour is more inspectable than black-box ML approaches.
...
\subsection{Evaluation Against Handbook Expectations}
The handbook expects a dissertation to present clear design rationale, implementation quality, critical evaluation, and defensible conclusions. The revised structure and expanded content address these expectations in three ways.

First, the report now follows a clear sequence from literature and technical review through methods, development/testing, outcomes analysis, discussion/evaluation, final conclusions, and future work. This sequence supports examiner readability and mirrors the recommended academic narrative.

Second, the report foregrounds evidence quality. Implementation claims are coupled with testing and benchmark evidence; unverified areas are explicitly marked as future work. This helps avoid overstatement and supports objective marking.

Third, the discussion explicitly weighs strengths against limitations and frames conclusions proportionately. The project is positioned as a robust prototype with meaningful research contribution, not a finished production platform. This positioning is appropriate for MSc scope and consistent with professional academic standards.
...
\textbf{Objective 6: Produce a comprehensive research report.} This objective was met through the revised dissertation structure, expanded technical narrative, evidence mapping, and handbook-aligned organisation. The report now provides traceable linkage from motivation and literature through design, implementation, validation, outcomes, discussion, conclusions, and future work.
...
\begin{enumerate}
    \item \textbf{For Practitioners:} When implementing agentic AI systems for structured operations, consider graph-based frameworks like LangGraph that provide deterministic routing without mandatory LLM dependencies.
    
    \item \textbf{For Researchers:} The hybrid architecture pattern demonstrated here (graph-based determinism with optional AI enhancement) warrants further investigation across different application domains.
    
    \item \textbf{For Framework Developers:} The evaluation highlights the value of cost-efficient, locally-runnable agent frameworks. Continued development of frameworks that support LLM-optional operation would benefit the community.
\end{enumerate}
...
This project has demonstrated that agentic AI principles can be effectively applied to learner support through careful framework selection and architecture design. The LangGraph-based implementation provides a foundation for future enhancements while delivering immediate value through structured task management, input validation, scheduling support, and proactive reminder capabilities including email delivery.
...
\begin{enumerate}
    \item Integration with real systems: future work could integrate the prototype with actual calendar applications or learning management systems, extending the supportive environment beyond the current standalone prototype.

    \item Multi-agent scenarios: exploring how multiple agentic support agents could coordinate when managing schedules for groups of learners or collaborative projects.
    ...
    \item Scalability evaluation: testing the prototype's performance with larger numbers of tasks (50+ tasks) or longer planning horizons (semester-level planning).
